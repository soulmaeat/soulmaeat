...
export const SignUp: FC = () => {
  ...
  const [isFormValid, setIsFormValid] = useState<boolean>(false); // 모든 필드가 유효한지 여부

  const navigate = useNavigate();

  // 입력 필드의 유효성 검사 및 모든 필드가 유효할 때 버튼 상태 업데이트
  useEffect(() => {
    const isUseridValid = /^([a-zA-Z])[a-zA-Z0-9!_-]{3,7}$/.test(user.userid);
    const isPasswdValid = /^[\w!_-]{4,8}$/.test(user.passwd);
    const isEmailValid = /^([A-Za-z])[\w-_]+(\.[\w]+)*@([a-zA-Z])+(\.)[a-z]{2,3}$/.test(user.email);
    const isPasswdMatch = user.passwd === passwordConfirm;
    const isSsNumValid = ssNum.length === 6 && ssNumSnd.length === 1;

    setIsFormValid(isUseridValid && isPasswdValid && isEmailValid && isPasswdMatch && isSsNumValid);
  }, [user.userid, user.passwd, user.email, passwordConfirm, ssNum, ssNumSnd]);
  ...
  const onSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      const response: AxiosResponse<ResponseUserData> = await axios.post(`${process.env.REACT_APP_API_URL}/api/register`, user);
    ...
    <section className="w-full max-w-3xl mx-auto text-center p-5 flex flex-col space-y-4">
      <form onSubmit={onSubmit} className="w-211 space-y-9">
        ...
        <div>
          <button
            type="submit"
            className={`w-full py-2 mb-3 text-2xl ${isFormValid ? 'bg-[#d75b22] text-white' : 'bg-[#f5f5f5] text-[#BDBDBD]'} font-semibold rounded-full`}
